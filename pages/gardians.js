...
export default function GardiansPage() {
  ...
  for (let i = 0; i < 9; i++) {
    gardiansImgs.push(`/collection-gardians/gardianLookbook/${i}.jpeg`);
  }
  ...
  //     console.log("visilibility status is", visibility);
  //     console.log('img source is', imageSrc)
  //   } else {
  ...
  //test
  function displayImg(i) {
    if (visibility === true || imageSrc === gardiansImgs[i]){
      setImageSrc('');
      setVisibility(false)
    } else {
      setImageSrc(gardiansImgs[i]);
  ...
  const imgBtns = gardiansImgs.map((data, i) => {
    return (
      <button onClick={() => displayImg(i)} className={styles.lookBtn}>
        -LOOK {i + 1}-
      </button>
    );
  });
  ...
  return (
    <>
      ...
      <div className={styles.modele}>
        <img
          className={styles.gardianImg1}
          src={"/collection-gardians/gardians-edito45LQ.jpeg"}
          alt="Edito image"
        />
      ...
      <div className={styles.modeleCollection}>
        <img
          ...
          src={"/collection-gardians/gardians-edito8LQ.jpeg"}
          alt="Edito image"
        />
